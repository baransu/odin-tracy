...
ZoneNC :: #force_inline proc(
	...
	ctx: ZoneCtx,
) {when TRACY_ENABLE {ctx = ZoneBegin(active, depth, loc); ZoneName(ctx, name); ZoneColor(
			ctx,
			color,
		)}
	return}
...
// Connection status
IsConnected :: #force_inline proc() -> bool {when TRACY_ENABLE {return(
			cast(bool)___tracy_connected() \
		)} else {return false}}
